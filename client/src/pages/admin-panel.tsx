...
export default function AdminPanel() {
  ...
  const { data: pendingSellers = [], isLoading: pendingSellersLoading } = useQuery({
    queryKey: ["/api/sellers/pending"],
  });

  const { data: approvedSellers = [], isLoading: approvedSellersLoading } = useQuery({
    queryKey: ["/api/sellers/approved"],
  });

  const { data: sellersWithDocuments = [], isLoading: sellersDocumentsLoading } = useQuery({
  ...
  const rejectProductMutation = useMutation({
    mutationFn: async ({ productId, reason }: { productId: string; reason: string }) => {
      return await apiRequest("POST", `/api/admin/products/${productId}/reject`, { reason });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/products/pending"] });
  ...
  const approveSellerDocumentsMutation = useMutation({
    mutationFn: async (userId: string) => {
      return await apiRequest("POST", `/api/admin/sellers/${userId}/approve-documents`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/sellers/documents"] });
  ...
  const rejectSellerDocumentsMutation = useMutation({
    mutationFn: async ({ sellerId, reason }: { sellerId: string; reason: string }) => {
      return await apiRequest("POST", `/api/admin/sellers/${sellerId}/reject-documents`, { reason });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/sellers/documents"] });
  ...
  const adminUploadDocumentMutation = useMutation({
    mutationFn: async ({ userId, field, fileUrl }: { userId: string; field: string; fileUrl: string }) => {
      return await apiRequest("PUT", `/api/admin/sellers/${userId}/upload-document`, { field, fileUrl });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/sellers/documents"] });
  ...
  const editSellerDetailsMutation = useMutation({
    mutationFn: async (sellerData: any) => {
      return await apiRequest("PUT", `/api/admin/sellers/${sellerData.userId}/edit-details`, sellerData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/sellers/documents"] });
  ...
  const handleAdminUpload = (userId: string, field: string, fileUrl: string) => {
  ...
  const filteredUsers = userRoleFilter === "all" ? users : (users as User[]).filter((u: User) => u.role === userRoleFilter);
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">Manage your marketplace platform</p>
          </div>
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              ...
              {(notifications as Notification[]).filter((n: Notification) => !n.isRead).length > 0 && (
                <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs">
                  {(notifications as Notification[]).filter((n: Notification) => !n.isRead).length}
                </Badge>
              )}
            </div>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                          {formatCurrency(parseFloat((analytics as any)?.totalRevenue || "0"))}
                        </p>
                      </div>
                ...
                <Card>
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-slate-600 dark:text-slate-400">Active Sellers</p>
                        <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                    ...
                    <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">{(pendingProducts as Product[]).length} pending review</p>
                        </div>
                      </div>
                      <Badge variant="secondary">{(pendingProducts as Product[]).length}</Badge>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">{(pendingSellers as User[]).length} pending approval</p>
                        </div>
                      </div>
                      <Badge variant="secondary">{(pendingSellers as User[]).length}</Badge>
                    </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">
                            {(sellersWithDocuments as SellerWithDocuments[]).filter((s: SellerWithDocuments) => s.status === "pending").length} pending review
                          </p>
                        </div>
                      </div>
                      <Badge variant="secondary">
                        {(sellersWithDocuments as SellerWithDocuments[]).filter((s: SellerWithDocuments) => s.status === "pending").length}
                      </Badge>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <ScrollArea className="h-64">
                    {pendingSellersLoading ? (
                      ...
                      <p className="text-slate-500 text-center py-8">No pending sellers</p>
                    ) : (
                      <div className="space-y-4">
                        {(pendingSellers as User[]).map((seller: User) => (
                          <div key={seller.id} className="p-3 border rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="font-medium">{seller.firstName} {seller.lastName}</h4>
                              <Badge variant="secondary" className="bg-orange-100 text-orange-800">
                                Pending
                              </Badge>
                            </div>
                            <p className="text-sm text-slate-600 mb-2">{seller.email}</p>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">{(approvedSellers as User[]).length}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ScrollArea className="h-64">
                    {approvedSellersLoading ? (
                      ...
                      <p className="text-slate-500 text-center py-8">No approved sellers</p>
                    ) : (
                      <div className="space-y-4">
          ...
          <TabsContent value="seller-documents" className="space-y-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Seller Document Verification</h3>
              <Badge variant="secondary">
                {(sellersWithDocuments as SellerWithDocuments[]).filter((s: SellerWithDocuments) => s.status === "pending").length} pending review
              </Badge>
            </div>
            ...
            {sellersDocumentsLoading ? (
              ...
              <div className="space-y-6">
                {(sellersWithDocuments as SellerWithDocuments[]).map((seller: SellerWithDocuments) => (
                  <Card key={seller.id}>
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-400">{seller.businessName}</p>
                          <p className="text-xs text-slate-500">
                          ...
                          <div className="flex space-x-2">
                            ...
                            {seller.status === "pending" && (
                              <>
                                <Button
                                  size="sm"
                                  onClick={() => approveSellerDocumentsMutation.mutate(seller.userId)}
                                  disabled={approveSellerDocumentsMutation.isPending}
                                >
                                  <CheckCircle className="h-4 w-4 mr-1" />
                                  Approve
                                </Button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div className="space-y-2">
                          <h5 className="font-medium text-slate-900 dark:text-slate-100">Business Details</h5>
                          <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
                            <p><strong>Type:</strong> {seller.businessType || "Not provided"}</p>
                            <p><strong>Phone:</strong> {seller.phoneNumber || "Not provided"}</p>
                            <p><strong>WhatsApp:</strong> {seller.whatsappNumber || "Not provided"}</p>
                            <p><strong>Address:</strong> {seller.businessAddress || "Not provided"}</p>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <h5 className="font-medium text-slate-900 dark:text-slate-100">Compliance Details</h5>
                          <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
                            <p><strong>Shop License:</strong> {seller.shopLicenseNumber || "Not provided"}</p>
                            <p><strong>Owner Civil ID:</strong> {seller.ownerCivilId || "Not provided"}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          ...
                          {seller.businessLogo ? (
                            <div className="p-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm text-green-700 dark:text-green-400">Uploaded</span>
                              </div>
                              <a 
                            ...
                            <div className="p-4 border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                              <p className="text-sm text-blue-700 dark:text-blue-400 mb-2">Admin Upload</p>
                              <FileUpload
                                onFilesUploaded={(files) => {
                                  if (files.length > 0) {
                                    handleAdminUpload(seller.userId, "businessLogo", files[0]);
                                  }
                                }}
                                accept="*/*"
                                maxFiles={1}
                          ...
                          {seller.shopLicenseImage ? (
                            ...
                            <div className="p-4 border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                              ...
                              <FileUpload
                                onFilesUploaded={(files) => {
                                  if (files.length > 0) {
                                    handleAdminUpload(seller.userId, "shopLicenseImage", files[0]);
                                  }
                                }}
                                accept="*/*"
                                maxFiles={1}
                        ...
                        {/* Owner Civil ID */}
                        <div className="space-y-2">
                          <Label className="text-sm font-medium">Owner Civil ID</Label>
                          {seller.ownerCivilIdImage ? (
                            ...
                            <div className="p-4 border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                              ...
                              <FileUpload
                                onFilesUploaded={(files) => {
                                  if (files.length > 0) {
                                    handleAdminUpload(seller.userId, "ownerCivilIdImage", files[0]);
                                  }
                                }}
                                accept="*/*"
                                maxFiles={1}
                        ...
                        {/* Owner Photo */}
                        <div className="space-y-2">
                          <Label className="text-sm font-medium">Owner Photo</Label>
                          {seller.ownerPhoto ? (
                            <div className="p-2 border rounded-lg bg-green-50 dark:bg-green-900/20">
                              <div className="flex items-center space-x-2">
                                <CheckCircle className="h-4 w-4 text-green-600" />
                            ...
                            <div className="p-4 border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                              ...
                              <FileUpload
                                onFilesUploaded={(files) => {
                                  if (files.length > 0) {
                                    handleAdminUpload(seller.userId, "ownerPhoto", files[0]);
                                  }
                                }}
                                accept="*/*"
                                maxFiles={1}
                  ...
                  <>
                    <div>
                      ...
                      <Input
                        ...
                        type="email"
                        defaultValue={(adminSettings as any)?.notification_email || ""}
                        className="mt-1"
                      />
                    </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Edit Seller Details</h2>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setEditingSellerData({
                        ...editingSellerData,
                  ...
                  <div>
                    <Label htmlFor="businessType" className="text-slate-700 dark:text-slate-300">Business Type</Label>
                    <Select
                      value={editingSellerData.businessType}
                      onValueChange={(value) => setEditingSellerData({
                        ...editingSellerData,
                        businessType: value
                      })}
                    >
                ...
                <div>
                  <Label htmlFor="businessAddress" className="text-slate-700 dark:text-slate-300">Business Address</Label>
                  <Textarea
                    id="businessAddress"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="shopLicenseNumber" className="text-slate-700 dark:text-slate-300">Shop License Number</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="ownerCivilId" className="text-slate-700 dark:text-slate-300">Owner Civil ID</Label>
                    <Input
                      id="ownerCivilId"
                ...
                <div className="flex justify-end space-x-2 pt-4 border-t border-slate-200 dark:border-slate-700">
                  ...
                  >
                    {editSellerDetailsMutation.isPending ? "Saving..." : "Save Changes"}
                  </Button>
                </div>
              </div>
            </div>
      ...
      {rejectDialogOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold mb-4">Reject {selectedProduct ? "Product" : "Documents"}</h3>
            <div className="space-y-4">
              <div>
                  ...
                  onClick={() => {
                    if (selectedProduct) {
                      rejectProductMutation.mutate({ 
                        ...
                        reason: rejectReason || "Product does not meet marketplace standards" 
                      });
                    } else if (selectedSeller) {
                      rejectSellerDocumentsMutation.mutate({ 
                        sellerId: selectedSeller, 
                        reason: rejectReason || "Documents do not meet verification requirements" 
                      });
                    }
                  }}
                  disabled={rejectProductMutation.isPending || rejectSellerDocumentsMutation.isPending}
                >
